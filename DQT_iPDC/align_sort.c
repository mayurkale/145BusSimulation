...
//Mayur: Below variables are added for application7
int frontk = -1;
int reark=-1;
int Tsb_Countk = 0;
int old_df_frontk = -1;
...
void time_alignk(struct data_frame *df) {
	...
	if (frontk == -1) { // TSB is used for the first time

		frontk = reark = 0;
		//TODO: implement below function
		assign_df_to_TSBk(df,frontk);
		...
		tsb_fsec[0] = TSBk[frontk].fracsec[1];
		tsb_fsec[1] = TSBk[frontk].fracsec[2];
		tsb_fsec[2] = TSBk[frontk].fracsec[3];

		tsb_soc = to_long_int_convertor((unsigned char *)TSBk[frontk].soc);

		tsb_fracsec = to_long_int_convertor1(tsb_fsec);
		IDcode = to_intconvertor(df->idcode);

		if((old_df_frontk != -1) &&  ((df_soc < ODFTk[old_df_frontk].soc) || ((df_soc == ODFTk[old_df_frontk].soc) && (df_fracsec < ODFTk[old_df_frontk].fracsec)))) {
		...
		for(i = 0; i<MAXTSBk; i++) {
			...
			if((TSBk[i].soc != NULL ) &&(TSBk[i].first_data_frame != NULL)) {

				tsb_soc = to_long_int_convertor((unsigned char *)TSBk[i].soc);
				tsb_fsec[0] = TSBk[i].fracsec[1];
				tsb_fsec[1] = TSBk[i].fracsec[2];
				tsb_fsec[2] = TSBk[i].fracsec[3];
				tsb_fracsec = to_long_int_convertor1(tsb_fsec);
	...
	//printf("I am first flag value %d\n",flag);
	if(flag) {

		//Assign to existing TSB
		//TODO
		assign_df_to_TSBk(df,i);

	} else {

		if(Tsb_Countk == MAXTSBk) {

			dispatch(frontk);

			frontk = (frontk + 1) % MAXTSBk;

			reark = (reark + 1) % MAXTSBk;
			assign_df_to_TSBk(df,reark);
			tsb_soc = to_long_int_convertor(TSBk[frontk].soc);
...
/* ----------------------------------------------------------------------------	*/
/* FUNCTION  assign_df_to_TSBk():                               	     		*/
/* It assigns the arrived data frame df to TSBk[index]				*/
/* ----------------------------------------------------------------------------	*/
void assign_df_to_TSBk(struct data_frame *df,int index) {
	/* Check if the TSB is used for the first time. If so we need to
		...
		if(TSBk[index].soc == NULL) { // 1 if

			struct  cfg_frame *temp_cfg = cfgfirst;

			TSBk[index].soc = malloc(5);
			TSBk[index].fracsec = malloc(5);

			memset(TSBk[index].soc,'\0',5);
			memset(TSBk[index].fracsec,'\0',5);

			copy_cbyc((unsigned char *)TSBk[index].soc,df->soc,4);
			copy_cbyc((unsigned char *)TSBk[index].fracsec,df->fracsec,4);
			TSBk[index].num = 0;

			// Kedar 06-05-2013 v1.4 initialize the old soc and fracsec
			unsigned char *fsec;
			fsec = malloc(3*sizeof(unsigned char));
			fsec[0] = TSBk[index].fracsec[1];
			fsec[1] = TSBk[index].fracsec[2];
			fsec[2] = TSBk[index].fracsec[3];
			...
			if(old_df_frontk == -1) {
				...
				ODFTk[old_df_reark].soc = to_long_int_convertor(TSBk[index].soc);
				ODFTk[old_df_reark].fracsec = to_long_int_convertor1(fsec);

			} else {

				old_df_reark = (old_df_reark + 1)%OLDFRAMECOUNTk;
				ODFTk[old_df_reark].soc = to_long_int_convertor(TSBk[index].soc);
				ODFTk[old_df_reark].fracsec = to_long_int_convertor1(fsec);
			}

			Tsb_Countk++;
			TSBk[index].first_data_frame = df; /* Assign df to the 'first_data_frame' in the data frame linked list of TSB[index] */
			TSBk[index].count = 1;
			...
			while(temp_cfg != NULL) {
				...
				if(TSBk[index].idlist == NULL) { /* Assign the pmuid to the idlist as it is the first id in the list */

					TSBk[index].idlist = temp_pmuid = pmuid;
			...
			TSBk[index].counter = 1;

			if(TSBk[index].count == TSBk[index].num) {

					int xx = index;
		//			writeTimeToLog(2,IDcode,df_soc,df_fracsec); // Checkpoint time before sorting
					intermediate_dispatchk(xx);
			} else {
		...
		//		writeTimeToLog(2,IDcode,df_soc,df_fracsec); // Checkpoint time before sorting
			...
			for (k=0;k<MAXTSBk;k++) {
				...
				if((k != index) && (TSBk[k].first_data_frame != NULL) && (TSBk[k].soc != NULL )) {

					TSBk[k].counter++;

					if(TSBk[k].counter == MAXTSBk) {

						intermediate_dispatchk(k);
					}
				}
			}
			//free(fsec);
			...
			struct  cfg_frame *temp_cfg = cfgfirst;
			if(TSBk[index].first_data_frame == NULL) { // 2 if
	...
	//				writeTimeToLog(2,IDcode,df_soc,df_fracsec); // Checkpoint time before sorting
			...
			//	pthread_mutex_unlock(&mutex_on_thread); // Added by KK on 19-Oct-2013
				...
				/* Need to check if df with same idcode and soc is already assigned to
				   the TSB[index] */
				check_df = TSBk[index].first_data_frame;
				while(check_df != NULL) {
				...
				temp_df = TSBk[index].first_data_frame;
	...
	unsigned int yy = xx;
		...
		if(frontk < reark) {
			...
			if(xx == reark) {
				...
				dispatchk(xx);
				reark= (reark - 1	)<0 ? reark-1+MAXTSBk:reark -1; // one step backward
			} else if(xx == frontk) {
				...
				while (xx < reark ) {
					...
					copy_cbyc (TSBk[xx].soc,(unsigned char *)TSBk[xx + 1].soc,4);
					copy_cbyc (TSBk[xx].fracsec,(unsigned char *)TSBk[xx + 1].fracsec,4);

					TSBk[xx].count = TSBk[xx + 1].count;
					TSBk[xx].num = TSBk[xx + 1].num;
					TSBk[xx].counter = TSBk[xx + 1].counter;
					TSBk[xx].idlist = TSBk[xx + 1].idlist;
					TSBk[xx].first_data_frame = TSBk[xx + 1].first_data_frame;
					xx++;
				}

				memset(TSBk[reark].soc,'\0',5);
				memset(TSBk[reark].fracsec,'\0',5);
				TSBk[reark].count = 0;
				TSBk[reark].num = 0;
				TSBk[reark].counter = 0;
				TSBk[reark].first_data_frame = NULL;
				TSBk[reark].idlist = NULL;

				reark= (reark - 1	)<0 ? reark-1+MAXTSBk:reark -1; // one step backward
			}
		} else if (frontk > reark) {

			if(xx == reark) {

				dispatchk(reark);

				if(frontk - reark == MAXTSBk)
				{
					reark = frontk;
				 } else {
					...
					while (rr != reark) {
						...
						copy_cbyc (TSBk[rr].soc,(unsigned char *)TSBk[h].soc,4);
						copy_cbyc (TSBk[rr].fracsec,(unsigned char *)TSBk[h].fracsec,4);

						TSBk[rr].count = TSBk[h].count;
						TSBk[rr].num = TSBk[h].num;
						TSBk[rr].counter = TSBk[h].counter;
						TSBk[rr].idlist = TSBk[h].idlist;
						TSBk[rr].first_data_frame = TSBk[h].first_data_frame;
						rr = (rr + 1) % MAXTSBk;
					}
...
void dispatchk(int index) {
	...
	int size,flag = 0;
	Tsb_Countk--;
	unsigned int tsb_soc = to_long_int_convertor(TSBk[index].soc);
	struct data_frame *df = TSBk[index].first_data_frame;
	if(df == NULL)
		printf("alarm1\n");
	else if(df->idcode == NULL)
		printf("alarm2\n");
	unsigned int IDcode = to_intconvertor(df->idcode);

	adjustOldFrameCountk(index);


	struct timeval tm;
	gettimeofday(&tm, NULL);

	Analysing(71,0,tm.tv_sec,tm.tv_usec);

	long int esoc = tm.tv_sec;
	long int efsec = tm.tv_usec;

	//printf("Yeh i am here in dispatchk data\n");
	size = create_dataframek(index);
	//printf("Yeh i am here after data\n");
	//printf("Yeh i am here in dispatchk data\n");
	//Below Analysing() is for noting time required for creating one data frame
	gettimeofday(&tm, NULL);
	Analysing(74,0,tm.tv_sec,tm.tv_usec);
	...
	while(temp_pdc != NULL ) {
		...
		if((temp_pdc->UL_upper_pdc_cfgsent == 1) && (temp_pdc->UL_data_transmission_off == 0)) {
			...
			if(temp_pdc->port == UDPPORT) {
				...
				if (sendto(temp_pdc->sockfd,dataframe, size+8, 0,
	...
	/*
	if(IamLPDC == true && flagApps == 1){
		exit(0);
	}
	/*		if(IamSPDC == true && flagApps >= numAppRunning){*/
	/*			exit(0);*/
	/*		}*/
...
void adjustOldFrameCountk(int index) {
	...
	tsb_fsec = malloc(3*sizeof(unsigned char));
	tsb_fsec[0] = TSBk[index].fracsec[1];
	tsb_fsec[1] = TSBk[index].fracsec[2];
	tsb_fsec[2] = TSBk[index].fracsec[3];

	tsb_soc = to_long_int_convertor((unsigned char *)TSBk[index].soc);
	tsb_fracsec = to_long_int_convertor1(tsb_fsec);

	for(i = 0; i< OLDFRAMECOUNTk; i++) {

		if((ODFTk[i].soc == tsb_soc) && (ODFTk[i].fracsec == tsb_fracsec)) {
		...
		if(old_df_frontk < old_df_reark) {
			...
			if(xx == old_df_reark) {
				...
				if(old_df_reark <0)
					...
					old_df_reark = old_df_reark + OLDFRAMECOUNTk;//

			} else if(xx == old_df_frontk) {

				old_df_frontk = (old_df_frontk + 1) % OLDFRAMECOUNTk;

			} else if(xx < old_df_reark)  {

				while (xx < old_df_reark) {

					ODFTk[xx].soc = ODFTk[xx + 1].soc;
					ODFTk[xx].fracsec = ODFTk[xx + 1].fracsec;
					xx++;
				}

				ODFTk[old_df_reark].soc = 0;
				ODFTk[old_df_reark].fracsec = 0;

				old_df_reark = (old_df_reark - 1) % OLDFRAMECOUNTk;
				if(old_df_reark < 0)
					old_df_reark = old_df_reark + OLDFRAMECOUNTk;
			}
		} else if (old_df_frontk > old_df_reark) {

			if(xx == old_df_reark) {

				if(old_df_frontk - old_df_reark == OLDFRAMECOUNTk)
				{
								//front = rear;
					old_df_reark = old_df_frontk;

				 } else {

					old_df_reark = old_df_reark - 1;
						if(old_df_reark <0)
							old_df_reark = old_df_reark + OLDFRAMECOUNTk;
				}
				...
				int rr = xx;
				while (rr != old_df_reark) {

					int h = (rr + 1) % OLDFRAMECOUNTk;
					ODFTk[rr].soc = ODFTk[h].soc;
					ODFTk[rr].fracsec = ODFTk[h].fracsec;
					rr = (rr + 1) % OLDFRAMECOUNTk;
				}
...
void clear_TSBk(int index) { //
	...
	TSBk[index].first_data_frame = NULL;
	TSBk[index].idlist = NULL;
	TSBk[index].num = 0;
	TSBk[index].counter = 0;
}
/* ----------------------------------------------------------------------------	*/
/* FUNCTION  create_dataframek():                              	     			*/
...
int create_dataframek(int index) {
	...
	byte_by_byte_copy(dataframe,(unsigned char *)TSBk[index].soc,z,4); //SOC
	z += 4;
	byte_by_byte_copy(dataframe,(unsigned char *)TSBk[index].fracsec,z,4); //FRACSEC + Time Quality?
	z += 4;
