...
void* connect_pmu_udp(void *temp) {
	...
	unsigned char *udp_BUF,*ptr,*ptr1,length[2];
	unsigned int flen;	
	uint16_t cal_crc,frame_crc;
	...
	cmdframe[18] = '\0';
	//printf("creating command frame for PMU %d\n",temp_pmu->pmuid);
	...
	if ((n = sendto(udp_sockfd,cmdframe, 18, 0, (struct sockaddr *)&PMU_addr,sizeof(PMU_addr)) == -1)) {
		...
		//printf("created command frame sending to %s lenth=%d\n",temp_pmu->ip,to_intconvertor(cmdframe+2));
		free(cmdframe);
		...
		while(1) {
			...
			if(bytes_read == -1) {
				...
				c >>= 5;
				//if(SPDCApp == true){
				//printf("char value %u socket : %d port: %d\n",c,udp_sockfd,port_num);
				//}

				if(c == 0x00){
					//printf("should not be here\n");
					int indx = bytes_read-8;
					unsigned char temp5[5],*d,mytemp;
					...
					if(SPDCApp == true) {
						if(code == 0x08){
							Analysing(10,id,pmusoc,pmufracsec); 
						}else if(code == 0x07){
							Analysing(65,id,pmusoc,pmufracsec);
						}else if(code == 0x09){
							app2_Analysing(22,id,soc,fsec,pmusoc,pmufracsec);

						}else{
							//app7pktsent++;
							Analysing(72,id,pmusoc,pmufracsec);
						}
					}else if(LPDCApp == true){

							Analysing(11,id,pmusoc,pmufracsec);

							Analysing(61,id,pmusoc,pmufracsec);

							app2_Analysing(20,id,soc,fsec,pmusoc,pmufracsec);

							Analysing(70,id,pmusoc,pmufracsec);
					}
				}			
				...
				if(flag) {
					...
					//printf("Before checking matching for IP \n");
					ptr = udp_BUF;
					ptr += 2;
					...
					if(frame_crc != cal_crc) {
						printf("Not matching for IP = %s length = %d \n",t->ip,flen);

						continue;
					}
					//					else
					//						printf("Matching for IP = %s length = %d UDPBUF[15]=%u \n",t->ip,flen,udp_BUF[15]);
					//process the frame									
