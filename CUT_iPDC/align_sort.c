...
/*printf("Dispatch\n");*/
		...
		Analysing(11,tm.tv_sec,tm.tv_usec);
		
		long int esoc = tm.tv_sec;
		long int efsec = tm.tv_usec;
		
/*		Dispatch_start_soc = esoc;*/
/*		Dispatch_start_fsec = efsec;*/
...
/*				VoltageStability(df,found);				*/
				pthread_t t;
				if((err = pthread_create(&t,&attr,VoltageStability,(void *)df))) {
					perror(strerror(err));		     
					exit(1);
				}
			}
...
/*				CoherencyOfGenerators();*/
				pthread_t t;
				if((err = pthread_create(&t,&attr,CoherencyOfGenerators,NULL))) {
					perror(strerror(err));		     
					exit(1);
				}
			}
...
/*				stateEstimator();*/
				pthread_t t;
				if((err = pthread_create(&t,&attr,stateEstimator,NULL))) {
					perror(strerror(err));		     
					exit(1);
				}
			}
...
//	printf("bingo11\n");
	...
	if(dataframe == NULL) {
		...
		dataframe = malloc((total_frame_size +1+8)*sizeof(unsigned char)); // Extra 8 bytes for Dispatch Time stamp
		if(!dataframe)
	...
	while(temp_df != NULL) { // 1
		...
		//printf("dataframe size = %d\n",to_intconvertor(temp_df->framesize));
        pmuKey = pmuInfoKey;
        entry.key = pmuKey;	
