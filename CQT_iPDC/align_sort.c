...
void dispatch(int index) {
	...
	if(IamSPDC == true) {
		...
		if(angleDiffApp == true) {
			...
			sprintf(appKey,"%d",1);
			entry.key = appKey;  
			hsearch_r(entry,FIND, &found,&hashForVSApp);	
			if(found != NULL)
...
//		printf("size = %d\n",size);	
	...
	while(temp_pdc != NULL ) {
		...
		if((temp_pdc->UL_upper_pdc_cfgsent == 1) && (temp_pdc->UL_data_transmission_off == 0)) {			
			...
			if(angleDiffApp == true && DDS == 2 && IamSPDC == true) {
			...
			} else if ((angleDiffApp == true) || (coherencyOfGen == true) && DDS == 2 && IamLPDC == true) {
				...
				byte_by_byte_copy(dataframe_DDS2,temp4,indx,4);
				indx = indx + 4;
				//printf("size of dataframe_DDS2 %d new size %u\n", size,sizeof(dataframe_DDS2));
				...
				if(temp_pdc->port == UDPPORT) {
					...
					if (sendto(temp_pdc->sockfd,dataframe_DDS2,size+8,0,(struct sockaddr *)&temp_pdc->pdc_addr,sizeof(temp_pdc->pdc_addr)) == -1)
							perror("sendto");					
...
int create_dataframe(int index) {
	...
	total_frame_size = COMBINE_DF_SIZE + 16 + 8;	/* 16 is for comman fields SYNC + FRAMESIZE + idcode + soc + fracsec + checksum + Analysis TS*/
